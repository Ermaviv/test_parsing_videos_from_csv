...
CTR = 15.0
RETENTION_RATE = 40
HEADERS_TEMPLATE = ['title', 'ctr', 'retention_rate', 'views', 'likes', 'avg_watch_time']
...
def validate_files_names(args):
    ...
    if args.files is None:
        raise ValueError('Не указаны файлы после параметра --files.')
    if args.report is None:
        raise ValueError('Не указано имя выходного файла после параметра --report.')
...
def extract_necessary_data(args, report_data_unsort, ctr_order):
    """
...
def sort_data(ctr_order, report_data_sort, report_data_unsort):
    """
...
def write_to_file(args, report_data_sort):
    """
    ...
    :return:
    """
    with open(args.report, 'w', encoding='utf-8') as write_file:
...
def module_main():
    ...
    headers = extract_necessary_data(args, report_data_unsort, ctr_order)
    sort_data(ctr_order, report_data_sort, report_data_unsort)
    write_to_file(args, report_data_sort)
